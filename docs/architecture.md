...
The current release implements the typed onboarding and leave-proposal graphs, JSON and SSE invocation, deterministic development notifications, transactional run/step/security-event recording, redacted Pino invocation logs, schedule/webhook/RabbitMQ onboarding triggers, and focused unit tests. Leave creation/approval, external notification providers, and external log shipping are later stories.

The leave worker starts the annual-policy and employee-balance tools together. Each tool independently resolves the canonical actor and target: HR may read any employee, while employees and managers may read only themselves. The worker counts Monday–Friday dates deterministically, excludes holidays by policy (the current sample has no holiday calendar entries), enforces three working days of notice and ten consecutive working days, and returns `requestCreated: false`; it has no leave-request write dependency.
