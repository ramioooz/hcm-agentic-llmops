...
| Symptom                                                        | Source-backed check and limit                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Management publication returns `{ "routed": false }`           | Confirm the exchange is `hcm.events.v1`, the routing key is `onboarding.review.requested`, and the onboarding queue binding exists. This result is a broker-routing failure; changing JSON payload fields cannot repair the route.                                                                                                                                                                                                                                                                                     |
| Publication is routed but no consumer lifecycle record appears | Confirm the API service is running, `AMQP_URL` reaches the broker, and `hcm.onboarding.review.v1` has a consumer. Then filter API logs by the broker `message_id` or correlation ID. Direct Management API publication does not create `rabbitmq.event.publish_confirmed`; the first application record is `rabbitmq.event.received`.                                                                                                                                                                                  |
| Retry or dead-letter confirmation fails                        | The transport waits for publisher confirmation before acknowledging the original delivery. A failed confirmation therefore leaves the original unacknowledged and eligible for redelivery. The runtime has no separate confirmation-failure lifecycle event, so use API/container failure logs and broker connection/channel evidence.                                                                                                                                                                                 |
| Attempt counts are unexpected                                  | The `x-attempt` header—not `processed_events.attempt`—drives the retry decision. Missing, invalid, or less-than-one values become `1`; successful retry publications increment it; `RABBITMQ_MAX_ATTEMPTS` defaults to `3`. Immediate deliveries can make stdout records interleave, so group them by message ID, correlation ID, and attempt.                                                                                                                                                                         |
| A message must be inspected in the DLQ                         | Use the Management UI with **Requeue**, RabbitMQ 4 `rabbitmqadmin get messages --queue hcm.onboarding.review.dlq.v1 --count 10 --ack-mode ack_requeue_true`, or the Management HTTP `get` request from the [manual scenario](manual-testing.md#mt-rabbitmq-02-validate-retry-attempts-and-non-destructively-inspect-the-dlq). Identify the target by `properties.message_id`; a bounded batch can include older entries. These options inspect and requeue only—there is no application replay/redrive implementation. |
...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Duplicate delivery     | The same payload and event ID become `DUPLICATE` while the claim is `COMPLETED` or `PROCESSING`; a `FAILED` claim is reclaimed for another attempt.                                                                                                       |
